...
//                    'data'   => json_encode($data, JSON_PRETTY_PRINT | JSON_THROW_ON_ERROR)
                ...
                flash_message('Login Failed',
                    count($this->validator->getErrors()) > 1
                        ? array_values($this->validator->getErrors())[0]
                        : 'Please correct the errors and try again.',
                    'error');

                return redirect()
                    ->to(route_to('login-view', $this->org_slug))
                    ->withInput();
            }
...
//                ->back()
            ...
            log_message('error', 'Error during login: {message} {stack}', [
                'message' => $e->getMessage(),
                'stack'   => $e->getTraceAsString()
            ]);

            $this->log_audit('Error during login for organization: ' . $this->org_slug, false, [
                'org_slug'          => $this->org_slug,
                'exception_message' => $e->getMessage()
            ]);

            flash_message('Login Error', 'An unexpected error occurred. Please try again later.', 'error');
