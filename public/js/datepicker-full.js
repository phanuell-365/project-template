(function () {
  ...
  <div class="datepicker-picker inline-block rounded-lg bg-white dark:bg-gray-700 shadow-lg p-4">
    ...
    <div class="datepicker-footer">
      <div class="datepicker-controls flex space-x-2 mt-2">
        <button type="button" class="%buttonClass% today-btn text-white bg-soko-700 dark:bg-soko-600 hover:bg-soko-800 dark:hover:bg-soko-700 focus:ring-4 focus:ring-soko-300 font-medium rounded-lg text-sm px-5 py-2 text-center w-1/2"></button>
        <button type="button" class="%buttonClass% clear-btn text-gray-900 dark:text-white bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-600 focus:ring-4 focus:ring-soko-300 font-medium rounded-lg text-sm px-5 py-2 text-center w-1/2"></button>
      </div>
    </div>
  </div>
</div>`);
  ...
  class DaysView extends View {
    ...
    render() {
      ...
      Array.from(this.grid.children).forEach((el, index) => {
        ...
        el.className = `datepicker-cell my-0.5 hover:bg-gray-100 dark:hover:bg-gray-600 block flex-1 leading-9 border-0 rounded-lg cursor-pointer text-center text-gray-900 dark:text-white font-semibold text-sm ${this.cellClass}`;
        ...
        if (this.range) {
          ...
          if (current > rangeStart && current < rangeEnd) {
            classList.add('fb-range', 'bg-gray-200', 'dark:bg-gray-600');
        ...
        if (this.selected.includes(current)) {
          classList.add('selected', 'bg-soko-700', 'text-white', 'dark:bg-soko-600', 'dark:text-white');
    ...
    refresh() {
      const [rangeStart, rangeEnd] = this.range || [];
      this.grid
        .querySelectorAll('.fb-range, .range-start, .range-end, .selected, .focused')
        .forEach((el) => {
          el.classList.remove('fb-range', 'range-start', 'range-end', 'selected', 'bg-soko-700', 'text-white', 'dark:bg-soko-600', 'dark:text-white', 'focused');
  ...
  class MonthsView extends View {
    ...
    refresh() {
      ...
      this.grid
        .querySelectorAll('.fb-range, .range-start, .range-end, .selected, .focused')
        .forEach((el) => {
          el.classList.remove('fb-range', 'range-start', 'range-end', 'selected', 'bg-soko-700', 'dark:bg-soko-600', 'dark:text-white', 'text-white', 'focused');
  ...
  class Datepicker {
    ...
    enterEditMode() {
      ...
      this.editMode = true;
      this.inputField.classList.add('in-edit', 'border-soko-700');
    }
    ...
    exitEditMode(options = undefined) {
      ...
      this.inputField.classList.remove('in-edit', 'border-soko-700');
      if (opts.update) {
        this.update(opts);
      }
    }
